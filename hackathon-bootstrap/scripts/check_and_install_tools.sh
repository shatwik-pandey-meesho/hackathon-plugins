...
git, docker, docker compose, node, npm, go, mysql client, gh, and gcloud.
...
install_macos() {
  ...
  brew install git node go gh mysql-client google-cloud-sdk
...
install_linux_apt() {
  sudo apt-get update
  sudo apt-get install -y git curl ca-certificates gnupg nodejs npm golang-go mysql-client docker.io docker-compose-plugin gh
...
status "mysql client" "command -v mysql"
