...
Container engine: the engine is Docker (Docker Desktop on macOS). This script installs
Docker here; on Windows the engine is Docker Desktop with the Hyper-V backend (see
check_and_install_tools.ps1 / ensure_container_engine.ps1).
USAGE
  exit 0
fi
...
install_macos() {
  ...
  echo "Docker Desktop may need to be opened once (open -a Docker) before Docker commands work."
}
