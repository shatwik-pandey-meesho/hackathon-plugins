...
- Use the provided script rather than hand-writing the login. It receives the token via the `HACKATHON_PROXY_TOKEN` env var (kept out of the invoking command's argv) and logs in with `docker login --password`. This is acceptable here because the token is only valid from the office IP; still never echo the token to the console or write it to memory/logs.
